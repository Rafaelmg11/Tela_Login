...
class Database:
    def __init__(self):
        ...
        self.conn = mysql.connector.connect(
            ...
            user = "root",
            password = "",
            database = "loginrafaelmagalhaes_db"
        )
    ...
    def RegistrarNoBanco(self,nome,email,usuario,senha):
        self.cursor.execute("INSERT INTO usuario (nome,email,usuario,senha)VALUES (%s,%s,%s,%s)",(nome,email,usuario,senha)) #INSERE OS DADOS DO USUARIO NA TABELA 
...
def alterar(self,idUsuario,nome,email,usuario,senha):
    self.cursor.execute("UPDATE usuario SET nome = %s,email = %s,usuario = %s,senha = %s WHERE idUsuario = %s",(nome,email,usuario,senha))#Atualiza os dados do usuario com o id fornecido
...
def exlcluir (self,idUsuario):
    self.cursor.execute("DELETE FROM usuario WHERE idUsuario = %s",(idUsuario,))#Exclui o usuario com o id fornecido
...
def buscar (self,idUsuario):
    self.cursor.execute("SELECT * FROM usuario WHERE idUsuario = %s",(idUsuario,))#Seleciona os dados do usuario com id fornecido
